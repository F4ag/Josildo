...
import { getRegistrationsByZoneSection, listDistinctRegistrationCities } from "@/services/reports"
import { PrintButton } from "@/components/print-button"
import { PrintLogo } from "@/components/print-logo"

export const metadata: Metadata = { title: "Cadastros por local, zona e seção · Lidera+" }

type SearchParams = { cidade?: string }

// Relatório separado do de "Cadastros por local de votação": aqui a linha é
// a combinação de Local de votação + Zona eleitoral + Seção eleitoral (as
// mesmas 3 informações que aparecem juntas na Ficha individual), não só o
// local. Ver services/reports.ts#getRegistrationsByZoneSection.
export default async function RelatorioLocalZonaSecaoPage({
  searchParams,
}: {
  ...
  const [{ rows, leadersWithoutInfo, supportersWithoutInfo }, cities] = await Promise.all([
    getRegistrationsByZoneSection(supabase, { city: params.cidade }),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Cadastros por local, zona e seção</h1>
        ...
        <div className="no-print flex items-center gap-2">
          <Link href={`/relatorios/local-zona-secao/pdf${pdfQuery ? `?${pdfQuery}` : ""}`}
      ...
      {(leadersWithoutInfo > 0 || supportersWithoutInfo > 0) && (
        <p className="text-xs text-foreground/50">
          {leadersWithoutInfo > 0 && <>{leadersWithoutInfo} liderança(s) </>}
          {leadersWithoutInfo > 0 && supportersWithoutInfo > 0 && "e "}
          {supportersWithoutInfo > 0 && <>{supportersWithoutInfo} apoiador(es) </>}
          ainda não têm local de votação, zona ou seção cadastrados e não entram nesta tabela.
        </p>
      )}
      ...
      <div className="no-print grid gap-3 sm:hidden">
        {rows.map((row) => (
          <div key={row.id} className="rounded-lg border border-black/5 bg-white p-4">
            <div className="mb-1 flex items-start justify-between gap-2">
              <p className="font-medium text-foreground">{row.pollingLocationLabel ?? "—"}</p>
              <span className="text-xs text-foreground/60">{row.totalCount} cadastro(s)</span>
            </div>
            <p className="text-xs text-foreground/50">
              {row.city ?? "—"} · Zona {row.zone ?? "—"} · Seção {row.section ?? "—"}
            </p>
        ...
        {rows.length === 0 && (
          <div className="rounded-lg border border-black/5 bg-white px-4 py-8 text-center text-sm text-foreground/50">
            Nenhum cadastro com local, zona ou seção informado ainda.
          </div>
        )}
        {rows.length > 0 && (
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-black/5">
                <td className="px-4 py-3 font-medium">{row.pollingLocationLabel ?? "—"}</td>
                <td className="px-4 py-3 text-foreground/70">{row.zone ?? "—"}</td>
                <td className="px-4 py-3 text-foreground/70">{row.section ?? "—"}</td>

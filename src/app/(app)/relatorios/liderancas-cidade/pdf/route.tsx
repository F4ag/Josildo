...
export async function GET(request: NextRequest) {
  ...
  const rows = await getLeadersByNeighborhoodReport(supabase, { city, sortBy: "city" })
  const buffer = await renderToBuffer(<LeadersReportDocument rows={rows} generatedAt={new Date()} groupBy="cidade" />)

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": 'attachment; filename="liderancas-por-cidade.pdf"',
    },
  })
}
